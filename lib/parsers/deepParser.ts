...
function buildCountMatcher(terms: string[]): RegExp {
  const escaped = terms.map(t => t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  return new RegExp(escaped.join('|'), 'gi');
}

const ANXIETY_RE = buildCountMatcher(ANXIETY_LEXICON);
const VALIDATION_RE = buildCountMatcher(VALIDATION_LEXICON);
const CONFESSIONAL_RE = buildCountMatcher(CONFESSIONAL_LEXICON);
const INTIMACY_RE = buildCountMatcher(INTIMACY_MARKERS);
const ATTACHMENT_ANXIOUS_RE = buildMatcher(ATTACHMENT_ANXIOUS);
const ATTACHMENT_AVOIDANT_RE = buildMatcher(ATTACHMENT_AVOIDANT);
const PERFECTIONISM_RE = buildCountMatcher(PERFECTIONISM_MARKERS);
const IMPOSTER_RE = buildCountMatcher(IMPOSTER_MARKERS);
const RUMINATION_RE = buildCountMatcher(RUMINATION_MARKERS);

// Pre-compile life event keyword matchers
const LIFE_EVENT_MATCHERS: Record<string, RegExp> = Object.fromEntries(
  Object.entries(LIFE_EVENT_SIGNALS).map(([id, cfg]) => [id, buildMatcher(cfg.keywords)])
);
...
function buildPsychologicalPortrait(messages: ScoredMessage[]): PsychologicalPortrait {
  ...
  if ((text.match(PERFECTIONISM_RE) || []).length >= 3) selfPerceptionThemes.push('Perfectionism / self-criticism');
  if ((text.match(IMPOSTER_RE) || []).length >= 2) selfPerceptionThemes.push('Imposter syndrome');
  if ((text.match(RUMINATION_RE) || []).length >= 3) selfPerceptionThemes.push('Chronic rumination');
